...
import { RESUME_PAGE_WIDTH } from '../../helpers/constants';
...
// 把设计稿的“间距体系”抽象成 token，便于在桌面/移动端按比例缩放
...
// 基于 space token 生成 Markdown 的渲染样式（font/padding/margin 等）
...
// resume-profile 是一个“自定义 code block”，用于在 Markdown 内嵌结构化信息
...
// 自定义 block renderer：把 resume-profile 代码块渲染成简历头图（头像 + 关键信息）
  ...
  return [
    {
      ...
      render: (ctx) => {
        ...
        } else if (data.avatar?.startsWith('src/')) {
        ...
        metaItems.push(...data.links.map((l) => ({ kind: 'link' as const, value: l })));
...
export class ResumeDemoApp extends StatelessWidget<ResumeDemoAppProps> {
  render() {
    ...
    if (mode === 'export') {
      // 导出模式：不使用 ScrollView，确保输出尺寸与排版稳定
      return (
        <Container
    ...
    return (
      ...
      >
        {/* 视图模式：页面水平居中，并保证 minWidth 让小屏也能正常滚动 */}
        <Container
          minWidth={width}
          alignment="center"
          color={theme?.background?.base ?? Themes.light.background.base}
