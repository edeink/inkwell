...
export default function Inkwell({ data, width = 600, height = 300, onError, onSuccess, readonly = false }: InkwellProps) {
  ...
  const compile = React.useCallback((src: string) => {
    ...
    const out = Babel.transform(wrapped, {
      plugins: [
        ...
        ['transform-react-jsx', { runtime: 'classic', pragma: 'createElement', pragmaFrag: 'Fragment' }],
      ],
    })
    return out.code || wrapped
  }, [])

  const renderData = React.useCallback(async (src: string) => {
    try {
      cleanup()
      if (!src || !src.trim()) {
        return
  ...
  React.useEffect(() => { void renderData(data) }, [data, renderData])
  ...
  React.useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      debounceTimerRef.current = window.setTimeout(() => {
        ...
        try { editorRef.current.rebuild() } catch { }
      }, 300)
    })
    ro.observe(el)
