...
export default function Charts({
 results 
  ...
  const byTest = useMemo(() => {
    ...
    const median = (arr: number[]) => { const s = [...arr].sort((a, b) => a - b); const m = Math.floor(s.length / 2); return s.length ? (s.length % 2 ? s[m] : (s[m - 1] + s[m]) / 2) : 0; };
 ...
 name, type: "line", data: byTest[name].map((v) => [v.nodes, v.createMs]), showSymbol: false 
}));
    return {
 title: {
 text: "总耗时(ms) vs 节点数" 
 ...
 type: "value", name: "ms" 
  ...
  const memOption = useMemo<EChartsOption>(() => {
    const series: any[] = Object.keys(byTest).map((name) => ({
 name, type: "line", data: byTest[name].map((v) => [v.nodes, v.mem]), showSymbol: false 
}));
    return {
 title: {
 ...
 type: "value", name: "bytes" 
  ...
  const fpsOption = useMemo<EChartsOption>(() => {
    const series: any[] = Object.keys(byTest).map((name) => ({
 name, type: "line", data: byTest[name].map((v) => [v.nodes, v.fps]), showSymbol: false 
}));
    return {
 title: {
 text: "刷新率(Hz) vs 节点数" 
  ...
  return (
    <>
      <div className={styles.grid}>
        <ReactECharts className={styles.chart} style={{
 height: 320 
}} option={timeOption} />
        <ReactECharts className={styles.chart} style={{
 height: 320 
