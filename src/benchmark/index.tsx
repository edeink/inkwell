...
type ProgressItem = { key: string; name: string; status: "pending" | "running" | "done"; current: number; total: number };
...
async function runSingleWithProgress(
  ...
  for (let i = 1; i <= total; i++) {
    ...
    samples.push({
 memory, metrics, frames 
});
    onProgress(i, total);
  }
  const avg = averageSamples(samples);
  return {
 name: test.name, mode: "compare" as any, samples, average: avg 
};
}
...
function useRunAll(stageRef: RefObject<HTMLDivElement>) {
  ...
  const [currentTask, setCurrentTask] = useState<{ name: string; round: number; total: number } | null>(null);
  ...
  const start = async () => {
    ...
    setProgressItems(tests.map((t) => ({
 key: t.name, name: t.name, status: "pending", current: 0, total: nodeCounts.length * repeat 
})));
    ...
    for (const t of tests) {
      ...
      for (const n of nodeCounts) {
        ...
        const res = await runSingleWithProgress(inst, n, repeat, (i, total) => {
          setCurrentTask({
 name: inst.name, round: i, total 
});
          setProgressItems((prev) => prev.map((it) => it.key === inst.name ? {
 ...it, status: "running", current: Math.min(done + i, it.total) 
} : it));
        });
        list.push(res);
        done += repeat;
        setProgressItems((prev) => prev.map((it) => it.key === inst.name ? {
 ...it, status: done === it.total ? "done" : "running", current: done 
} : it));
      }
    }
    setResults(list);
  ...
  return {
 loading, results, start, stop, progressItems, currentTask, showStage, nodeCounts, setNodeCounts, repeat, setRepeat 
};
}
...
function App() {
 ...
 width: 240 
          ...
          />
          <Space>
            <Button type="primary" onClick={start} disabled={loading}>运行测试</Button>
            <Button danger onClick={stop} disabled={!loading}>停止</Button>
          </Space>
        </Space>
      </div>
      <Charts results={results} />
      <Modal open={showStage} footer={null} centered maskClosable={false} width="100vw" styles={{
 body: {
 padding: 0 
} 
}} onCancel={stop}>
