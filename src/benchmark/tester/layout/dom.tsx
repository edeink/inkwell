 ...
 * - Nodes:
 *   - Even (Flex): simulates Container(padding) > Center. Uses box-sizing: border-box to contain padding.
 *   - Odd (Absolute): simulates Stack > Positioned > Container(border). Fills parent via top/left/right/bottom.
...
export async function createLayoutDomNodes(stage: HTMLElement, count: number): Promise<Timings> {
  ...
  root.style.cssText =
    'width: 100%; height: 100%; display: flex; flex-wrap: wrap; align-content: flex-start; gap: 2px; overflow: hidden; background: #fff; box-sizing: border-box;';
  ...
  for (let i = 0; i < chains; i++) {
    ...
    for (let d = 0; d < DEPTH; d++) {
      ...
      if (d % 2 === 0) {
        ...
        node.style.cssText =
          'display: flex; justify-content: center; align-items: center; padding: 1px; width: 100%; height: 100%; position: relative; box-sizing: border-box;';
      } else {
        // Odd: Stack -> Positioned(1,1,1,1) -> Container(border).
        // Absolute positioning with t/l/r/b mimics Positioned constraints.
        node.style.cssText =
          'position: absolute; top: 1px; left: 1px; right: 1px; bottom: 1px; border: 1px solid rgba(0,0,0,0.1); box-sizing: border-box;';
      }
    ...
    leaf.style.cssText = `width: 100%; height: 100%; background-color: ${
      i % 2 ? '#4caf50' : '#2196f3'
