...
describe('性能优化', () => {
  it('性能测试：buildChildren 与 对象池复用', () => {
    ...
    const root = new PerfItem({ key: 'root' });
    const count = 5000;
    ...
    root.createElement({ key: 'root', children: childrenData });
    const firstBuild = performance.now() - start;
    testLogger.log(`首次构建 (${count} items): ${firstBuild.toFixed(2)}ms`);

    // 更新：完全复用
    const updateStart = performance.now();
    root.createElement({ key: 'root', children: [...childrenData] });
    const updateTime = performance.now() - updateStart;
    testLogger.log(`完全复用更新 (${count} items): ${updateTime.toFixed(2)}ms`);
    ...
    root.createElement({ key: 'root', children: partialData });
    const partialTime = performance.now() - partialStart;
    testLogger.log(`部分更新 (${count} items): ${partialTime.toFixed(2)}ms`);
    ...
    root.createElement({ key: 'root', children: childrenData }); // 重新添加原始 keys
    const fullTime = performance.now() - fullStart;
    testLogger.log(`从对象池恢复 (${count} items): ${fullTime.toFixed(2)}ms`);

    testLogger.log('--- 性能监控报告 ---');
    testLogger.log(getMetricsReport());
