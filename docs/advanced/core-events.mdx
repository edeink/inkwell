...
从 vNext 起，事件处理支持两种等效绑定：
- JSX 属性：`<Text onClick={handler} onClickCapture={capHandler} />`
- 类方法：在组件类上定义同名方法，如 `onClick()`、`onClickCapture()`

一致性保证：
- 触发顺序完全一致：捕获 → 目标（捕获）→ 目标（冒泡）→ 冒泡
- 事件对象 `InkwellEvent` 属性与方法一致（`type/currentTarget/eventPhase/x/y/stopPropagation()`）
- 阻止传播效果一致：返回 `false` 或调用 `e.stopPropagation()` 均生效
- 共存时优先级：类方法优先于同一节点上的 JSX 属性处理器
...
(<Container key="root" width={240} height={120}>
  {/* 假设 CustomWidget 已注册为组件类型 */}
  <CustomWidgetElement key="leaf" width={80} height={40} onClick={() => console.log('attr:leaf')} />
</Container>)
````

当 `CustomWidget` 同时实现 `onClick()` 方法时，点击顺序为：`method:leaf → attr:leaf`。
...
### 最佳实践
- 简单场景优先使用 JSX 属性，直观且与渲染结构贴近
- 组件内需要复用/覆写时使用类方法，保证封装性与可维护性
- 同一节点避免同时定义过多处理器，确保性能与可读性
