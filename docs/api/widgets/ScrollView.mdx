...
<ScrollView width={360} height={240}>
  <Column>
    <Container width={360} height={120} color="#f0f0f0">
      <Text text="外层：垂直滚动容器（向下滚动）" />
    </Container>

    <ScrollView width={360} height={120}>
      <Row>
        {Array.from({ length: 12 }).map((_, i) => (
          <Container key={i} width={120} height={120} color={i % 2 === 0 ? '#e6f7ff' : '#fff7e6'}>
            <Center>
              <Text text={`卡片 ${i + 1}`} />
            </Center>
          </Container>
    ...
    <Container width={360} height={240} color="#f0f0f0">
      <Text text="继续向下滚动：外层会接管；到达边界后交给页面滚动" />
    </Container>
  </Column>
</ScrollView>
```
