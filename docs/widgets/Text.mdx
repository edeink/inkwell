...
<Container width={360} padding={8} border={{ width: 1, color: '#ddd' }}>
  <Column spacing={8} mainAxisSize="min">
    <Text text="默认：行高等于字号" fontSize={16} />

    <Text text="指定行高 24px（lineHeight）" fontSize={16} lineHeight={24} />

    <Text text="使用 height 作为行高（同义字段）" fontSize={16} height={28} />

    <Text text={"多行省略：多行文本示例，多行文本示例，多行文本示例，多行文本示例，多行文本示例"} fontSize={16} lineHeight={22} maxLines={3} overflow="ellipsis" />
