...
```mermaid
graph TD
  A["MindmapDemo（React）"] --> B["InkwellCanvas"]
  A --> C["MindmapContext.Provider"]

  B --> D["Runtime"]
  D --> E["runApp"]
  E --> F["MindmapDemo（Widget）"]

  F --> G["MindMapViewport"]
  G --> H["MindMapLayout"]
  H --> I["MindMapNode"]
  H --> J["Connector"]
  H --> K["MindMapNodeToolbar"]
  F --> L["MindMapEditorOverlay"]

  D --> M["MindmapController"]
  M --> N["ViewModule"]
  M --> O["InteractionModule"]
  M --> P["LayoutModule"]
...
1.  **React 层（UI 外壳与辅助交互）**
    *   `src/demo/mindmap/index.tsx`：挂载 `InkwellCanvas`，负责创建 `Runtime`、重建 App、初始化 Controller 与主题订阅。
    *   `src/demo/mindmap/components/*`：`Toolbar`、`ZoomBar`、`Minimap` 等，依赖 `MindmapController` 实现 UI 控制与状态显示。

2.  **Controller（视图状态与交互协调）**
    *   `src/demo/mindmap/controller/index.ts`：桥接 Runtime 与 Viewport，对外提供 `zoomAt`、`setViewPosition`、`addViewChangeListener` 等稳定 API。
    *   `src/demo/mindmap/controller/modules/*`：按职责拆分（视图、交互、布局），便于扩展与测试。

3.  **Widget 层（渲染与命中）**
    *   `src/demo/mindmap/app.tsx`：`MindmapDemo`（`StatefulWidget`），维护图数据与编辑态，并渲染 `MindMapViewport` / `MindMapLayout` / `MindMapNode` / `Connector` 等。
    *   `src/demo/mindmap/widgets/*`：节点、连接线、布局容器、视口，以及 DOM 覆盖层相关组件（编辑器、工具栏）。
...
导图的核心状态集中在 `MindmapDemo(Widget)`：图数据（节点/边）、选中与编辑态，以及 Viewport 的 `scale/tx/ty`。当 Viewport 发生变换时，通过回调同步到 Controller，再由 Controller 广播给 React 层订阅者（例如缩略图与缩放条）。
...
*   `src/demo/mindmap/index.tsx`: React 挂载入口（Runtime、主题订阅、Controller 初始化）。
*   `src/demo/mindmap/app.tsx`: 画布场景（`StatefulWidget`）与核心状态管理。
*   `src/demo/mindmap/controller/`: 控制器与模块化扩展点。
*   `src/demo/mindmap/widgets/`: Viewport、Layout、Node、Connector、编辑器覆盖层等渲染组件。
*   `src/demo/mindmap/components/`: ZoomBar、Minimap、Toolbar 等 React UI 组件。
